...
create index if not exists idx_transport_outbox_cleanup on transport_outbox_messages(status, type, created_at_ms);
...
create index if not exists idx_transport_inbox_cleanup on transport_inbox_messages(status, type, created_at_ms);
-- +goose StatementEnd
