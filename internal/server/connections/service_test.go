...
func (f *fakeProvider) ApplyConnection(context.Context, remote.ApplyInput) error {
	return nil
}
...
func TestSelectEnrollmentNodesAllOnlySupportedActiveNodes(t *testing.T) {
	...
	activeNative, err := st.CreateNode(model.Node{Name: "native", Type: model.NodeTypeNativeHy2})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(nodes) != 3 {
		t.Fatalf("expected three supported active nodes, got %#v", nodes)
	}
	got := map[string]bool{}
	for _, node := range nodes {
		got[node.ID] = true
	}
	if !got[activeBlitz.ID] || !got[activeAmnezia.ID] || !got[activeNative.ID] {
		t.Fatalf("expected selected nodes to include active Blitz, Amnezia and native Hysteria nodes, got %#v", nodes)
	}
}
...
func TestCreateEnrollmentConnectionCreatesBlitzAccessAndConfigs(t *testing.T) {
	...
	svc.providerFor = func(model.Node, remote.AgentControl) (remote.Provider, error) { return fake, nil }
